...
import { getProject, updateProjectMassiveTicker } from "@/lib/store/projects";
...
export async function POST(
  ...
  const updated = await updateProjectMassiveTicker(guard.user.id, id, { tickerInfo, fetchedAt });
  if (!updated) {
    return NextResponse.json({ error: "Unable to store ticker info" }, { status: 500 });
  }

  return NextResponse.json({ tickerInfo: updated.massiveTickerInfo, fetchedAt });
}
